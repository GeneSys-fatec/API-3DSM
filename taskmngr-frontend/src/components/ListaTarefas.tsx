...
export default function ListaTarefas() {
    ...
    const carregarTarefas = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Erro na requisição: ${response.statusText}. Resposta: ${errorBody}`);
            }
    ...
    const executarExclusao = useCallback(async () => {
        ...
        try {
            const response = await authFetch(`http://localhost:8080/tarefa/apagar/${tarefaParaExcluir}`, {
                method: "DELETE",
            });

            if (!response.ok) {
                const errorBody = await response.text();
                throw new Error(`Erro ao excluir tarefa: ${response.statusText}. Resposta: ${errorBody}`);
            }
            setTarefas(prevTarefas => prevTarefas.filter(tarefa => tarefa.tar_id !== tarefaParaExcluir));
    ...
    if (!selectedProjectId) {
         return <div className="p-4 text-center text-gray-500">Selecione um projeto para ver as tarefas.</div>;
    }
        ...
        <>
            <div className="bg-white md:p-4 md:rounded-lg md:shadow-md overflow-hidden relative min-w-full">
                ...
                <div className="hidden md:block">
                            ...
                            {tarefas.map((tarefa, index) => (
                                <div key={tarefa.tar_id} className="grid grid-cols-12 gap-3 p-3 items-center hover:bg-gray-100 transition-all duration-100 ease-in-out">
                                    ...
                                    <div className="col-span-1 flex justify-center">
                                        <span className={`px-2 py-1 text-xs font-bold rounded-md uppercase ${getPrioridadeClass(tarefa.tar_prioridade)}`}>{tarefa.tar_prioridade}</span>
                                    </div>
                                    <div className="col-span-1 overflow-hidden flex justify-center">
                                        <span className={`px-2 py-1 text-xs font-bold rounded-md uppercase truncate ${getStatusClass(tarefa.tar_status)}`} title={tarefa.tar_status}>
                                            {tarefa.tar_status}
                                        </span>
                                    </div>
                                    <div className="col-span-1 flex justify-center">
                                        <button onClick={() => abrirModalEdicao(tarefa)} className="text-xs font-semibold bg-gray-200 text-gray-700 px-2 py-1 rounded-md hover:bg-gray-300 transition-colors">
                                            Editar
                                        </button>
                                    </div>
                                    <div className="col-span-1 flex justify-center">
                                        <button className="text-xs font-semibold bg-gray-200 text-gray-700 px-2 py-1 rounded-md hover:bg-gray-300 transition-colors" onClick={() => setTarefaParaExcluir(tarefa.tar_id)}>
                                            Excluir
                                        </button>
                                    </div>
                                </div>
                    ...
                    <button onClick={abrirModalCriacao} className="text-sm font-semibold text-blue-600 hover:text-blue-800 pt-4 cursor-pointer">
                ...
                <div className="block md:hidden relative w-screen -ml-1">
                        ...
                        <div className="space-y-4">
                            {tarefas.map((tarefa, index) => (
                                <React.Fragment key={tarefa.tar_id}>
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex-1 min-w-0 mr-3">
                                                ...
                                                <div className="flex gap-2">
                                                    <span className={`px-3 py-1 text-sm font-bold rounded-full uppercase ${getPrioridadeClass(tarefa.tar_prioridade)}`}>
                                                        {tarefa.tar_prioridade}
                                                    </span>
                                                    <span className={`px-3 py-1 text-sm font-bold rounded-full uppercase ${getStatusClass(tarefa.tar_status)}`}>
                                                        {tarefa.tar_status}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="flex gap-1 flex-shrink-0">
                                                <button onClick={() => abrirModalEdicao(tarefa)} className="text-blue-600 hover:text-blue-800 p-2" title="Editar">
                                                    <i className="fa-solid fa-pencil text-lg"></i>
                                                </button>
                                                <button className="text-red-600 hover:text-red-800 p-2" onClick={() => setTarefaParaExcluir(tarefa.tar_id)} title="Excluir">
                                    ...
                                    {index < tarefas.length - 1 && <hr className="border-gray-400 mx-4" />}
                                </React.Fragment>
                            ))}
                        </div>
                    </div>
                    <div className="fixed bottom-20 left-4 right-4 z-50">
                        <div className="bg-white border-t border-gray-200 p-3 rounded-lg shadow-lg">
                            <button onClick={abrirModalCriacao} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-4 rounded-lg transition-colors flex items-center justify-center text-lg" title="Adicionar Nova Tarefa">
                    ...
                    <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
                        <div className="text-center">
                            ...
                            <div className="flex gap-4 justify-center" style={{ paddingTop: '20px' }}>
                                <button onClick={() => setTarefaParaExcluir(null)} className="px-6 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors font-medium">
                                    Cancelar
                                </button>
